...
//        return Collections.emptyIterator();
    ...
    public Iterator<RecordId> scanGreaterEqual(DataBox key) {
        ...
        // TODO(proj2): Return a BPlusTreeIterator.
        return root == null ? Collections.emptyIterator() : new BPlusTreeIterator(key);
    }
    ...
    private class BPlusTreeIterator implements Iterator<RecordId> {
        ...
        public BPlusTreeIterator() {
            ...
            curIterator = curLeafNode.scanAll();
        }
        public BPlusTreeIterator(DataBox key) {
            curLeafNode = root.getLeftmostLeaf();
            this.scanMethod = ScanMethod.GREATER_EQUAL;
            this.key = key;
            curIterator = curLeafNode.scanGreaterEqual(key);
        }
        @Override
        public boolean hasNext() {
            // TODO(proj2): implement
            // 如果当前节点还有剩余 or 还有下一个节点, 则还有next
            return curIterator.hasNext()|| testHasNext(curLeafNode.getRightSibling());
        }
...
//                return num;
        ...
        private boolean testHasNext(Optional<LeafNode> leafNode){
            ...
            LeafNode node = leafNode.get();

            // 如果存在且大小为正数, 则返回true
            Iterator<RecordId> recordIdIterator = scanMethod.equals(ScanMethod.ALL) ? node.scanAll() : node.scanGreaterEqual(key);
            if(recordIdIterator.hasNext()){
                return true;
            }
            // 如果为空则需看右邻居是否hasNext
            return testHasNext(node.getRightSibling());
        }
    }
}
