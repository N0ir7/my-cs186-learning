...
class LeafNode extends BPlusNode {
    ...
    private Optional<Pair<DataBox, Long>> myPut(DataBox key, RecordId rid,float fillFactor,float retainFactor) {
        ...
        List<RecordId> newRIDList = new ArrayList<>(subRIDList);
        ...
        // 生成新的 leaf node
        LeafNode newLeafNode = new LeafNode(metadata, bufferManager, newKeysList, newRIDList, rightSibling, treeContext);

        // 修改当前node 的 right sibling
        long newPageNum = newLeafNode.page.getPageNum();
    ...
    public static LeafNode fromBytes(BPlusTreeMetadata metadata, BufferManager bufferManager,
        ...
        // 读取键值对
        List<DataBox> keysList = new ArrayList<>(keyNum);
        List<RecordId> ridList = new ArrayList<>(keyNum);
        for (int i = 0; i < keyNum; i++) {
            keysList.add(DataBox.fromBytes(buffer,metadata.getKeySchema()));
            ridList.add(RecordId.fromBytes(buffer));
        }

       return new LeafNode(metadata,bufferManager,page,keysList,ridList,Optional.ofNullable(rightSibling),treeContext);
    }
